# Agentic Retrieval Augmented Generation (RAG) (Tìm kiếm và tạo câu trả lời mang tính tác nhân)

Trong Chương này, chúng ta sẽ cùng tìm hiểu cách sử dụng Agentic RAG để giúp Alfred chuẩn bị cho buổi tiệc gala tuyệt vời.

<Tip>Chúng mình biết các bạn đã được học về Retrieval Augmented Generation (RAG) và agentic RAG ở Chương trước, vì vậy hãy thoải mái bỏ qua phần này nếu đã nắm vững các khái niệm.</Tip>

LLM (mô hình ngôn ngữ lớn) được training trên kho dữ liệu khổng lồ để học kiến thức tổng quát.  
...
Giống như Alfred cần tìm kiếm thông tin trong gia đình bạn để hỗ trợ, bất kỳ Agent (tác nhân) nào cũng cần cách hiểu và tìm kiếm dữ liệu liên quan.  
**Agentic RAG là phương pháp mạnh mẽ để sử dụng các Agent trả lời câu hỏi về dữ liệu của bạn.** Ta có thể cung cấp nhiều Tools (công cụ) khác nhau để Alfred trả lời câu hỏi.  
