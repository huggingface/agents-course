...
今日のAIで最もエキサイティングなトピック、**エージェント**へようこそ！
...
## コースの内容はどのようになっていますか? [[course-look-like]]
...
| :---- | :---- | :---- |
| 0 | Onboarding | 使用するツールやプラットフォームのセットアップを行います。 |
| 1 | Agent Fundamentals | ツール、思考、行動、観察、およびその形式について説明します。LLM、メッセージ、特殊トークン、チャットテンプレートについて説明します。ツールとしてPython関数を使用した簡単なユースケースを示します。 |
| 2 | Frameworks | 基礎がどのように人気のあるライブラリ(smolagents、LangGraph、LLamaIndex)に実装されているかを理解します。 |
| 3 | Use Cases | 実際のユースケースを構築します(経験豊富なエージェントビルダーからのPRを歓迎します🤗)。 |
| 4 | Final Assignment | 選択したベンチマーク用のエージェントを構築し、学生リーダーボードでエージェントの理解を証明します🚀。 |
...
## 推奨ペースはどのくらいですか? [[recommended-pace]]
