...
3. **Start Ollama in the background (In one terminal)**
    ...
    service, so you can skip this command to start Ollama. For the same error, on windows you can identify the process using `netstat -ano | findstr :11434` the number on the rightmost column will indicate process id.
