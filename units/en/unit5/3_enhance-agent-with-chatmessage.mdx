...
# Example with metadata dictionary for displaying thoughts
...
@dataclass
class ChatMessage:
   content: str | Component  # Text or media component
...
- The MetadataDict is a typed dictionary to represent metadata for a message in the Chatbot component. An instance of this dictionary is used for the metadata field in a ChatMessage when the chat message should be displayed as a thought.

```python
class MetadataDict(TypedDict):
   title: str # Title of the thought bubble
   id: int | str # For nested thought, Agents
...
One of the most powerful features of Agents is their ability to reason through problems. With `gr.ChatMessage`, we can make this thinking visible:

```python
import gradio as gr
from gradio import ChatMessage

def reasoning_agent(query, history):
    """
    Example of using agents with reasoning LLM like DeepSeek-r1 
    """
...
...
demo.launch()
...
< Add a gif of the app deployed on Spaces >
