...
# Tool schema for prompting
tool_schema = {
    "name": "guest_info_tool",
    "description": "Retrieve information about a guest by their full name.",
    "parameters": {
        "type": "object",
        "properties": {
            "guest_name": {
                "type": "string",
                "description": "The exact full name of the guest"
            }
        },
        "required": ["guest_name"]
    }
...
system_prompt = f"""You are Alfred, a helpful assistant.
When you need guest information, respond with ONLY valid JSON matching this exact schema:

{json.dumps(tool_schema, indent=2)}

No extra text, no markdown, no explanation.
If you have enough information or no tool is needed, respond normally."""
...
def assistant(state: AgentState):
    ...
    response = chat.invoke(full_messages)
    content = response.content.strip()
    
    # Attempt to parse JSON tool call
    try:
        tool_call = json.loads(content)
        if tool_call.get("name") == "guest_info_tool":
            guest_name = tool_call["parameters"].get("guest_name", "")
            result = guest_info_tool.invoke(guest_name)  # Tool expects string
            
            return {
                "messages": [
                    AIMessage(content=content),
                    ToolMessage(content=json.dumps(result), tool_call_id="call_1")
                ]
            }
...
def should_continue(state: AgentState):
    ...
    return "assistant" if isinstance(last_msg, ToolMessage) else END
