...
class CodeAgentWithCustomMemory(CodeAgent):
...
system_prompt = (
    "Create a concise summary (50 words max per element of the summary) of this conversation. "
    "Focus on key entities, facts, and context needed for future reference."
)

alfred_with_memory = CodeAgentWithCustomMemory(
    tools=[guest_info_tool, weather_info_tool, hub_stats_tool, search_tool], 
    model=model,
    add_base_tools=True,
    planning_interval=3,
