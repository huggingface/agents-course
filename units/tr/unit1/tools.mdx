...
## AI Tools (Yapayu Zeka Araçları) Nedir?
...
Basitleştirilmiş bir **calculator** aracı gerçekleştireceğiz, bu iki tam sayıyı çarpacak. Python kodumuz şböyle olabilir:
...
Bu dizeyi LLM'e giriş olarak verdiğimizde, model bunu bir araç olarak tanır ve hangi girdileri vermesi gerektiğini ve ne tür bir çıktı beklemesi gerektiğini bilir.
...
- Docstring yorumuyla sağlanan uzun açıklama: `Multiply two integers.`
- Girdi türleri: iki int
- Çıktı türü.

İnsanlar programlama dillerini bu yüzden kullanıyor: açık, özlü ve kesinler.

Python kaynak kodunu araca ait _specification_ (spesifikasyon) olarak verebiliriz, ancak aracın nasıl yazıldığı önemli değil. Önemli olan adı, ne yaptığı, hangi girdileri beklediği ve hangi çıktıyı verdiğidir.
...
Sonrasında, sadece `@tool` dekoratörünü kullanarak `calculator` fonksiyonunun bir araç olduğunu belirtmemiz yeterli:
...
Bu açıklama sistem prompt'una **eklenir**. Bu bölümün başındaki örneği ele alırsak, `tools_description` yerini alarak şu şekilde görünür:
...
[Eylemler](actions) bölümünde, Agent'ın bu oluşturduğumuz aracı nasıl Call ettiğini (çağırdığını) daha ayrıntılı öğreneceğiz.
