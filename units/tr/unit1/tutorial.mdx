...
Ayrıca oluşturduğunuz Agent'ı **bir HuggingFace Space üzerinde yayınlayarak arkadaşlarınızla ve iş arkadaşlarınızla paylaşabileceksiniz**.
...
Space'i kopyaladıktan sonra, Agent'ınızın model API'sine erişebilmesi için HuggingFace API token'ınızı eklemeniz gerekecek:

1. Eğer hala yoksa, [https://hf.co/settings/tokens](https://hf.co/settings/tokens) adresinden çıkarım (inference) izni olan bir HuggingFace token'ı alın  
