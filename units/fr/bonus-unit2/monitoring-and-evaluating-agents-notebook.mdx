...
with propagate_attributes(
    ...
    with langfuse.start_as_current_observation(name="Smolagent-Trace") as span:
        # Exécutez votre application ici
        response = agent.run("Quelle est la capitale de l'Allemagne ?")

        # Mettez à jour le span avec l'entrée et la sortie
        span.update(input="Quelle est la capitale de l'Allemagne ?", output=response)
